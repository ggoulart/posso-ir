...
export class GermanyClient {
  ...
  public async fetchTravelBan(): Promise<string> {
    return this.request
      .get('https://brasil.diplo.de/br-pt/coronavirus/2320108')
      .then((resp) => {
  ...
  private static parseAppointmentResp(data: string): boolean {
    ...
    const controlText =
      'Sistema de marcação online do Ministério Federal das Relações Externas &mdash; Porto Alegre Voltar atrás Selecione uma categoria'
    return parsedText !== controlText
  }

  private static parseTravelBanResp(data: string): string {
    const output = /Esta proibição de entrada e transporte foi prorrogada até (\d{2}\/\d{2}\/\d{4}) e poderá ser novamente prorrogada dependendo do desenvolvimento da pandemia/.exec(
      data
    )
    if (output) {
      return output[1]
